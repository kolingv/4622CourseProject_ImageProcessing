  ...
  main(int argc, char *argv[])
{
  if (argc != 5)
    {
      fprintf(stderr,"Usage: %s <in bmp file> <out bmp file> N a \n",argv[0]);
      return -1;
    }
  ...
  try {
	  ...
	  // Allocate storage for the RGB output height*weight
	  my_image_comp *RGBoutput_comps = new my_image_comp[3];
	  for(n=0; n < 3; n++)
		  RGBoutput_comps[n].init(height, width, 0); // No extension required
	  ...
	  //symetric extension
	   ...
	   int out_r, out_c;
	   for(out_r=0; out_r < height; out_r++)
		   for(out_c=0; out_c < width; out_c++)
		   {
			   int p1=out_c, p2=out_r; 
			  ...
			  for (n=0; n < num_comps; n++)
				{
				 ...
				 // printf("normolize finished!\n");
				  //re-arrange the quadrants			 					  
					  float *gamma = gamma_comps[n].buf + N/2*(N+1) + N/2;  // point to center
					  //Q1
					  for(r=0; r <= N/2; r++)
						 for(c=0; c <= N/2; c++)
						 {
					...
					//	RGBoutput_comps[0].buf[r*RGBoutput_comps[0].stride + c]=avgR;
						RGBoutput_comps[2].buf[out_r*width+out_c] = avgR;
						// printf("R= %f\t", avgR);
						//G
						for(r=-N/2; r <= N/2; r++)
							for(c=-N/2; c <= N/2; c++)
						...
						RGBoutput_comps[1].buf[out_r*width + out_c]=avgG;
						// printf("G= %f\t", avgG);
						//B
						for(r=-N/8; r <= N/8; r++)
							for(c=-N/8; c <= N/8; c++)
						...
						RGBoutput_comps[0].buf[out_r*width + out_c]=avgB;
						// printf("B= %f\n", avgB);
				}
      ...
      bmp_out out;
	  io_byte *new_line = new io_byte[width * 3];
          ...
          for (n=0; n < 3; n++)
            {
              ...
              float *src = RGBoutput_comps[n].buf + r*RGBoutput_comps[n].stride;
