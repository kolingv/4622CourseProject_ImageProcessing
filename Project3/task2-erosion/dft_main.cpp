  ...
  main(int argc, char *argv[])
{
  ...
  if ((argc -1) % 2 != 0) // pairs of elements(vectors) 
    {
      fprintf(stderr,"Usage: %s <in bmp file> <out bmp file> vector(s)\n",argv[0]);
      return -1;
    }
  ...
  try {
	  ...
	  for(i=0; i < aoff_size; i++, j += 2)
		  A_x1[i] = atoi(argv[3+j]);
	  j=0;
	  for(i=0; i < aoff_size; i++, j += 2)
		  A_x2[i] = atoi(argv[4+j]);
      ...
      for (n=0; n < num_comps; n++)
        input_comps[n].init(height,width,border); // border extension required
