  ...
  try {
	  ...
	  */
	  ...
	  for (n=0; n < num_comps; n++)
		...
		//complement the original pic
		  for (n=0; n < num_comps; n++)
		  for(r=0; r < height; r++)
			  for(c=0; c < width; c++)
			  { 
				if(input_comps[n].buf[r * input_comps[n].stride + c] > 200)
					input_comps[n].buf[r * input_comps[n].stride + c]=0;
				else 
					input_comps[n].buf[r * input_comps[n].stride + c]=255;
			  }
	  ...
	  my_image_comp *intermed_coms = new my_image_comp[num_comps];
	  for (n=0; n < num_comps; n++)
	  {
       intermed_coms[n].init(out_height,out_width,R); // R extension required
														//because erosion may be found at boundary
	  }

	  //initialize out_put with 0
	  for (n=0; n < num_comps; n++)
		  for(r=0; r < out_height; r++)
			  for(c=0; c < out_width; c++)
			  {
				intermed_coms[n].buf[r*intermed_coms[n].stride + c] = 0;
			  }
	   ...
	   for (n=0; n < num_comps; n++)
		   for(r=0; r < height; r++)
			   for(c=0; c < width; c++)
			   {
				 ...
				 float *op = intermed_coms[n].buf + r * intermed_coms[n].stride + c;
				 int val = 255;
				 for(i=0; i < aoff_size; i++)
				 {	
				 ...
				 //intermed_coms[n].buf[r * intermed_coms[n].stride + c] = val;
				 //dilation
				 if(val != 0)
				 {
					  for(i=0; i < aoff_size; i++)
	  ...
	  //transfer pixel value to output
	  for(n=0; n < num_comps; n++)
		  for(r=0; r < out_height; r++)
			  for(c=0; c < out_width; c++)
			  {
				output_comps[n].buf[r*output_comps[n].stride + c]=
					intermed_coms[n].buf[r*intermed_coms[n].stride + c];
			  }
